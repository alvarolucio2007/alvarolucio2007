## Hi there/ Olá / Hallöchen 👋

![Github Streak](https://github-readme-streak-stats.herokuapp.com/?user=SEU_USER&theme=tokyonight)

## 🇺🇸 English
I am a Backend Developer in training, graduating in Software Engineering, focused on **Python**, and trying other languages, such as **Java**, and in the future, **Typescript** and **Go**.
- 🛠 **Environment:** Pop!_OS Linux 24.04 + LazyVim
- 🔭 **Currently working on:** Personal CRUD projects and College 
- ⚡ **Fun fact:** I can do all things through Christ, who strenghtens me.

## 🇧🇷 Português
Sou um desenvolvedor Backend em formação,graduando em Engenharia de Software, com foco em **Python**, e experimentando outras linguagens, como **Java**, e futuramente, **Typescript** e **Go**
- 🛠️ **Ambiente:** Pop!_OS Linux 24.04 + LazyVim
- 🔭 **Trabalhando em:** Projetos CRUD pessoais e projetos da Faculdade.
- ⚡ **Curiosidade:** Tudo posso Naquele que me fortalece.

## 🇩🇪 Deutsch
Ich bin ein angehender Backend-Entwickler sowie Software Engineering Student, mit Fokus auf **Python**. Momentan probiere ich jetzt **Java** aus und plane für die Zukunft **Typescript** und **Go**.
- 🛠️ **Umgebung:** Pop!_OS Linux 24.04 + Lazyvim
- 🔭 **Ich arbeite an:** Persönlichen CRUD-Projekten und Uni-Projekten
- ⚡ **Fun Fact:** Ich vermag alles durch den, der mich mächtig macht, Christus.

![Metrics](https://metrics.lecoq.io/SEU_USER)
![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)
![Lazyvim](https://img.shields.io/badge/LazyVim-2E7DE9.svg?style=for-the-badge&logo=lazyvim&logoColor=white)
![Pop!_OS](https://img.shields.io/badge/Pop!_OS-48B9C7?style=for-the-badge&logo=Pop!_OS&logoColor=white)
